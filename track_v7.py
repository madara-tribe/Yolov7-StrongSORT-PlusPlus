...
from yolov7.utils.general import check_img_size, check_requirements, check_imshow, apply_classifier, strip_optimizer, set_logging, increment_path
            ...
            if len(det):
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        bboxes, id, cls = get_boxes_result(output)
                        if draw:
                        ...
                        if post:
                            results.append([frame_idx + 1, id, output[0], output[1], output[2], output[3]])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--post', action='store_true', help='for AFlink(offline processing) and GSI(post-processing) after StrongSort')
